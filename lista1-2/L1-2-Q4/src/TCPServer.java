	...
	public void run() {
		while(true) {
			...
			if(data[0].equals("CALC")) {
					...
					switch(data[1]){
						case "ADD":
							res = Calculadora.getInstance().soma(val1, val2);
						break;
						case "SUB":
							res = Calculadora.getInstance().sub(val1, val2);
						break;
						case "MULT":
							res = Calculadora.getInstance().mult(val1, val2);
						break;
						case "DIV":
							res = Calculadora.getInstance().div(val1, val2);
						break;
						default:
				...
				sendResponse(Conversor.getInstance().converter(data[1], Float.parseFloat(data[2])));
